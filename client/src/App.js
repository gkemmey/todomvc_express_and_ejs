...
const NewTodo = ({ refresh }) => {
  ...
  const createTodo = () => {
    return post("/todos", { todo: { title: title } }).
...
const Todo = ({ id, title, completed, refresh }) => {
  ...
  const updateTodo = (todo) => {
    return patch(`/todos/${id}`, { todo: todo }).then((res) => { if (res.ok) { refresh() } })
  }

  const destroyTodo = () => {
    destroy(`/todos/${id}`).then((res) => { if (res.ok) { refresh() }})
  }
...
const Todos = ({ todos, refresh }) => {
  const updateAll = (todo) => {
    patch(`/todos/update_many`, { ids: todos.map((t) => ( t.id )), todo: todo }).
      then((res) => { if (res.ok) { refresh() } })
  }
...
const Footer = ({ search, todos, refresh }) => {
  ...
  const destroyCompleted = () => {
    destroy("/todos/destroy_many", { ids: todos.filter((t) => ( t.completed )).map((t) => ( t.id )) }).
      then((res) => { if (res.ok) { refresh() } })
  }
...
const App = ({ location: { search } }) => {
  ...
  useEffect(() => {
    const path = search.includes("completed") ?
      `/todos?completed=${search.includes("completed=true")}`:
      '/todos'
